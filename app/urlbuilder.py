...
def build_search_url(year_term, dept_name, show_finals, show_comments):
    query_paramaters = [('YearTerm', year_term + '-92'), ('ShowFinals', show_finals),
                        ('ShowComments', show_comments), ('Dept', dept_name)]

    return BASE_URL + '?' + urllib.parse.urlencode(query_paramaters)
